...
query GetAccountByRiotId($gameName: String!, $tagLine: String!) {
    getAccountByRiotId(gameName: $gameName, tagLine: $tagLine)
        @rest(type: "RiotAccount", path: "/riot/account/v1/accounts/by-riot-id/{args.gameName}/{args.tagLine}", method: "GET") {
        puuid
        gameName
        tagLine
    }
...
export const useGetAccountByRiotId = () => {
    const [getAccount, { data, loading, error }] = useLazyQuery<GetAccountByRiotIdResponse, GetAccountByRiotIdVariables>(
...
query GetMatchesByPuuid($puuid: String!) {
    getMatches(puuid: $puuid) @rest(type: "[String]", path: "/lol/match/v5/matches/by-puuid/{args.puuid}/ids")
}
```
